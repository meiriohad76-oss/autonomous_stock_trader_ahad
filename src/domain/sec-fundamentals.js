...
const SIC_SECTOR_RULES = [
  { sector: "Information Technology", patterns: [/software/i, /computer/i, /semiconductor/i, /electronic/i, /data processing/i] },
  { sector: "Communication Services", patterns: [/communications/i, /broadcast/i, /media/i, /telecommunications/i, /internet/i] },
  { sector: "Consumer Discretionary", patterns: [/retail/i, /hotel/i, /restaurant/i, /apparel/i, /automobile/i, /consumer electronics/i] },
  { sector: "Consumer Staples", patterns: [/food/i, /beverage/i, /household/i, /tobacco/i, /drug stores/i] },
  { sector: "Health Care", patterns: [/pharmaceutical/i, /biotech/i, /medical/i, /health/i, /diagnostic/i] },
  { sector: "Financials", patterns: [/bank/i, /insurance/i, /investment/i, /financial/i, /credit/i] },
  { sector: "Industrials", patterns: [/industrial/i, /transportation/i, /air freight/i, /machinery/i, /aerospace/i, /railroads/i] },
  { sector: "Energy", patterns: [/oil/i, /gas/i, /energy/i, /drilling/i, /pipeline/i] },
  { sector: "Materials", patterns: [/chemical/i, /mining/i, /metals/i, /paper/i, /materials/i] },
...
function inferSectorFromSicDescription(sicDescription, fallbackSector = "Unknown") {
  const text = String(sicDescription || "").trim();
  if (!text) {
    return fallbackSector || "Unknown";
  }

  for (const rule of SIC_SECTOR_RULES) {
...
function hydrateCompanyIdentity(baseCompany, submissions, cik, filing = null) {
  const industry = submissions?.sicDescription || baseCompany.industry || "Pending SEC classification";
  return {
    ...baseCompany,
    cik: submissions?.cik || cik || baseCompany.cik,
    company_name: submissions?.name || baseCompany.company_name,
    sector: inferSectorFromSicDescription(submissions?.sicDescription, baseCompany.sector),
    industry,
    filing_date: filing?.filing_date || baseCompany.filing_date,
    period_end: filing?.period_end || baseCompany.period_end,
    form_type: filing?.form_type || baseCompany.form_type,
    filing_url: filing?.source_url || baseCompany.filing_url
  };
}
    ...
    try {
      ...
      const processed = await mapWithConcurrency(trackedCompanies, config.fundamentalSecConcurrency || 4, async (company) => {
        ...
        try {
          ...
          if (!filing) {
            return {
              company: hydrateCompanyIdentity(company, submissions, cik),
              live: false,
              artifact: null,
              error: false
            };
          ...
          return {
            company: hydrateCompanyIdentity(company, submissions, cik, filing),
            live: false,
            artifact: null,
            error: false
          };
