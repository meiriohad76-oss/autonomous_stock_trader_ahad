...
The sentiment aggregator uses `downstream_weight` and `display_tier` before computing state. Alerts suppress low-value duplicate evidence. Trade setups use average evidence quality to adjust conviction. The dashboard exposes the same verdict in feeds and signal drawers.
