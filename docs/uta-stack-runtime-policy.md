...
| Frontend | React/Vite target | The canonical production target is a compiled frontend under `/uta`. The first replay slice may use static ES modules until npm package installation is healthy. |
| Durable UTA data | SQLite WAL on SSD/NVMe | Store baselines, observations, signal results, replay runs, and user state. |
